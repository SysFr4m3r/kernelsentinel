...
#   ks-run: lab
#
# Writes inside the container's own /etc, so the path the kernel reports is the
# watched one. The lab image pre-creates /etc/cron.d precisely so a missing
# directory cannot make this pass by writing nothing.
set -euo pipefail
