...
| **Full sensor set** | 5.11 with `CONFIG_BPF_LSM=y` **and** `bpf` in `/sys/kernel/security/lsm` — both, not either. Compiled-in but inactive gets you five of thirteen |
...
startup and, when `bpf` is missing, reports the eight `lsm/` sensors as
unavailable with that as the reason — `5 of 13 sensors active`, not `13 of 13`.
...
| socket_listen | `lsm/` | 5.11 + BPF-LSM | ✗ |
...
|---|---|---|---|
| **Kali rolling** | 7.1 | ✅ active by default | **verified** — `kali-2026.3`, kernel 7.1.5, 13/13 sensors active |
